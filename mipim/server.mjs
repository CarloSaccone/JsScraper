...
async function parsePage(page, ruleSet) {
  ...
  const superfici = await page.evaluate((ruleSet) => {
    ...
    return [Array.from(items, (element) => element.textContent), items.length];
  }, ruleSet);
  ...
  var pricesmq = superfici[0].map((i, index) => {
    ...
    var price = prezzi[0][index]
      .replace(ruleSet.prezzo.replace, "")
      .replace(/\./g, "");
  ...
  var hasNext = false;
  try {
    hasNext = await page.waitForSelector(
      "DIV.in-pagination.in-searchList__pagination > DIV.in-pagination__control > A.in-pagination__item > svg"
    );
